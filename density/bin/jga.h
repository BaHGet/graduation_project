...
#define fact_max    100               // maxium n stored in fact[n]=n!
#define fact2_max   200               // maxium n stored in fact2[n]
double  *fact,                        // fact[n] = n!
        *fact2,                       // fact2[n]=(n!)^(1/2)
        *ffact,                       // ffact[n]= n!!
...
int read_int(char *command)
{
  ...
  if(!strcmpC(command,"false"))      return 0;
  if(!strcmpC(command,"true"))       return 1;
                                     return atoi(command);
}
...
char *ftostr(double x, int d, int opt)
{
...
char *ftostr(double x, int d) {return ftostr(x,d,0);}
...
void on_syntax_error(char *where, char *com)
  {fprintf(foutput,"*** syntax error in '%s': %s\n",where,com);  exit(1);}

void on_syntax_error(char *where, char *c1, char *c2)
